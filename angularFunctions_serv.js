...
app.controller('myCtrl', function($scope, $http) {
  ...
  $scope.updUsrLoc = function(loc) {
    ...
    $http.post('https://ec2-18-219-170-175.us-east-2.compute.amazonaws.com:8080/api/addLoc', payload).then(function(response) {
  ...
  $scope.verify2 = function() {
    ...
    if (document.getElementById('verNumbr').value == $scope.user.verf) {
      ...
      $http.post('https://ec2-18-219-170-175.us-east-2.compute.amazonaws.com:8080/api/logEntry', payload).then(function(response) {
  ...
  $scope.verify = function() {
    ...
    if (string == "footnotead1@gmail.com") {
      ...
      //  console.log(' $scope.user.id', payload.userID);
      $http.post('https://ec2-18-219-170-175.us-east-2.compute.amazonaws.com:8080/api/mail', payload).then(function(response) {
      ...
      //  console.log(' $scope.user.id', payload.userID);
      // $http.post('https://ec2-52-15-85-14.us-east-2.compute.amazonaws.com:8080/api/mail', payload).then(function(response) {
      //   console.log("mail sent");
      // }, function(error){
      //   console.log(error);
      // });
      ...
      console.log('breakpoint4');
        ...
        $http.post('https://ec2-18-219-170-175.us-east-2.compute.amazonaws.com:8080/api/mail', payload).success(function (data){
          console.log("mail sent");
  ...
  $scope.getMessage = function() {
    ...
    $http.post('https://ec2-18-219-170-175.us-east-2.compute.amazonaws.com:8080/api/retrieve', [$scope.coords]).then(function(response) {
  ...
  $scope.getInitialMessage = function(coords) {
    ...
    // polyPoints = window.polyPoints;
    $http.post('https://ec2-18-219-170-175.us-east-2.compute.amazonaws.com:8080/api/retrieve', [coords]).then(function(response) {
      //  $scope.respMessages.push(response.data[0].messageContent);
  ...
  $scope.getMyMessages = function(user, path) {
    ...
    // polyPoints = window.polyPoints;
    $http.post('https://ec2-18-219-170-175.us-east-2.compute.amazonaws.com:8080/api/retrieveMe', [hash(user),path]).then(function(response) {
      //  $scope.respMessages.push(response.data[0].messageContent);
  ...
  $scope.send = function() {
    ...
    if ($scope.user.numberOfPosts > $scope.user.threshold) {
      ...
      $http.post('https://ec2-18-219-170-175.us-east-2.compute.amazonaws.com:8080/api/add', payload ).then(function(response) {
