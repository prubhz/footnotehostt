...
app.controller('myCtrl', function($scope, $http) {
  ...
  $scope.updUsrLoc = function(loc) {
    ...
    $http.post('http://localhost:3000/api/addLoc', payload).then(function(response) {
  ...
  $scope.verify2 = function() {
    ...
    if (document.getElementById('verNumbr').value == $scope.user.verf) {
      ...
      $http.post('http://localhost:3000/api/logEntry', payload).then(function(response) {
  ...
  $scope.verify = function() {
    ...
    if (string == "footnotead1@gmail.com") {
      ...
      //  console.log(' $scope.user.id', payload.userID);
      $http.post('http://localhost:3000/api/mail', payload).then(function(response) {
  ...
  $scope.getMessage = function() {
    ...
    $http.post('http://localhost:3000/api/retrieve', [$scope.coords]).then(function(response) {
  ...
  $scope.getInitialMessage = function(coords) {
    ...
    // polyPoints = window.polyPoints;
    $http.post('http://localhost:3000/api/retrieve', [coords]).then(function(response) {
      //  $scope.respMessages.push(response.data[0].messageContent);
  ...
  $scope.getMyMessages = function(user, path) {
    ...
    // polyPoints = window.polyPoints;
    $http.post('http://localhost:3000/api/retrieveMe', [hash(user),path]).then(function(response) {
      //  $scope.respMessages.push(response.data[0].messageContent);
  ...
  $scope.send = function() {
    ...
    if ($scope.user.numberOfPosts > $scope.user.threshold) {
      ...
      $http.post('http://localhost:3000/api/add', payload).then(function(response) {
